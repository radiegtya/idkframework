...
In this example we are going to find records with connection from `posts` table. Basically it's the same as [**`find-all`**](./1-find-all.mdx), but only it have additional detail as records `count`, `skip`, and `take`.
...
```md title="[GET]"
/api/v1/posts/connection?where[title][contains]=better
```
...
```md title="[GET]"
/api/v1/posts/connection?select[title]=true&select[content]=true
```
...
```md title="[GET]"
/api/v1/posts/connection?select[title]=true&select[content]=true&select[createdBy][select][email]=true
```
...
```md title="[GET]"
/api/v1/posts/connection?include[createdBy]=true
```
