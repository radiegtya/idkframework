...
In this example we are going to find records with connection from `posts` table. Basically it's the same as [**`find-all`**](./1-find-all.mdx), but only it have additional detail as records `count`.

- When using **REST API**, instead of using regular `/api/v1/posts`, we are going to use `/api/v1/posts/connection`.
- When using **Javascript SDK**, instead of using `find` method, we are going to use `findConnection` method.
