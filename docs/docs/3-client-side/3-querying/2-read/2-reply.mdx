...
<Tabs>
  <TabItem value="rest_api" label="REST API" default>
  To <b>read</b> all records with <b>filter</b> above using REST API, you have to send a <code>GET</code> request to <code>/replies</code> endpoint with stringified object query params <code>where[title][contains]=good</code>
...
<BrowserWindow url="/replies?where[title][contains]=News">
...
<BrowserWindow url="/replies?select[title]=true&select[content]=true">
